...
namespace DotLiquid.FileSystems
{
    ...
    /// ie. with the template name prefixed with an underscore. The extension ".bwt" is also added.
    /// 
    ...
    /// 
    /// file_system.full_path("mypartial") # => "/some/path/_mypartial.bwt"
    /// file_system.full_path("dir/mypartial") # => "/some/path/dir/_mypartial.bwt"
    /// </summary>
    public class LocalFileSystem : IFileSystem
	{
		...
		public string FullPath(string templatePath)
		{
			...
			string fullPath = templatePath.Contains("/")
				? Path.Combine(Path.Combine(Root, Path.GetDirectoryName(templatePath)), string.Format("_{0}.bwt", Path.GetFileName(templatePath)))
				: Path.Combine(Root, string.Format("_{0}.bwt", templatePath));
